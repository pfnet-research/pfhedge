...
class CryptoDataLoader:
    ...
    def load_perpetual_data(self, filename: Optional[str] = None) -> pd.DataFrame:
        ...
        if filename is None:
            ...
            if not files:
                raise FileNotFoundError(f"No perpetual data files found in {self.data_dir}")
            filename = files[0]
        else:
    ...
    def load_options_data(self, filename: Optional[str] = None) -> pd.DataFrame:
        ...
        if filename is None:
            ...
            if not files:
                raise FileNotFoundError(f"No options data files found in {self.data_dir}")
            filename = files[0]
        else:
            filename = self.data_dir / filename
    ...
    def _process_perpetual_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(df) > 5 and 'log_returns' in df.columns:
            df['volatility_5'] = df['log_returns'].rolling(window=5).std() * np.sqrt(24)  # Annualized
    ...
    def _process_options_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'expiration' in df.columns:
            ...
            df['time_to_expiry'] = (df['expiration'] - df['timestamp']).dt.total_seconds() / (365.25 * 24 * 3600)
    ...
    def get_price_series(self, frequency: str = '5T') -> pd.DataFrame:
        ...
        price_series = df[['last_price', 'bid_price', 'ask_price', 'mid_price']].resample(frequency).last()
        ...
        price_series['log_returns'] = np.log(price_series['last_price'] / price_series['last_price'].shift(1))

        # Calculate volatility
        price_series['volatility'] = price_series['log_returns'].rolling(window=20).std() * np.sqrt(365.25 * 24 * 60 / 5)

        return price_series.reset_index()

    def get_options_for_expiry(self, expiry_date: Optional[datetime] = None) -> pd.DataFrame:
        """
        ...
        if expiry_date is None:
            ...
            df['days_to_expiry'] = (df['expiration'] - current_time).dt.total_seconds() / (24 * 3600)
    ...
    def create_backtest_dataset(
        ...
        # Get options data
        options = self.options_data.copy() if self.options_data is not None else pd.DataFrame()
    ...
    def summary(self) -> Dict:
        ...
        if self.perpetual_data is not None:
            ...
            summary['perpetual'] = {
                ...
                'avg_spread_pct': perp['spread_pct'].mean() if 'spread_pct' in perp.columns else None
            }
        ...
        if self.options_data is not None:
            ...
            summary['options'] = {
                'records': len(opts),
                'unique_strikes': opts['strike'].nunique() if 'strike' in opts.columns else 0,
                'call_count': len(opts[opts['option_type'] == 'call']) if 'option_type' in opts.columns else 0,
                'put_count': len(opts[opts['option_type'] == 'put']) if 'option_type' in opts.columns else 0,
...
def test_loader():
    ...
    try:
        ...
        if atm_call:
            print(f"ATM call: strike={atm_call['strike']}, IV={atm_call.get('mark_iv', 'N/A')}")

        # Test backtest dataset
        dataset = loader.create_backtest_dataset()
        print(f"Backtest dataset: {len(dataset['prices'])} price points, {len(dataset['options'])} options")
