...
def run_experiment(config):
    ...
    print(f"Training complete: {initial_loss:.2f} → {final_loss:.2f} ({improvement:.1f}% improvement)")
    ...
    sharpe_improvement = (deep_sharpe - bs_sharpe) / abs(bs_sharpe) * 100 if bs_sharpe != 0 else 0

    print(f"\nResults:")
    print(f"  Deep PnL: ${deep_mean:,.2f} ± ${deep_std:,.2f} (Sharpe: {deep_sharpe:.3f})")
    print(f"  BS PnL:   ${bs_mean:,.2f} ± ${bs_std:,.2f} (Sharpe: {bs_sharpe:.3f})")
    print(f"  Improvements: Mean {mean_improvement:+.1f}%, Risk {std_improvement:+.1f}%, Sharpe {sharpe_improvement:+.1f}%")
...
def run_optimization_suite():
    ...
    for result in results:
        ...
        indicator = "✅" if mean_imp > 0 and deep_sharpe > result['black_scholes']['sharpe'] else "⚠️ "

        print(f"{indicator} {name:<28} ${deep_mean:>10.2f} {deep_sharpe:>10.3f} {mean_imp:>10.1f}%")
