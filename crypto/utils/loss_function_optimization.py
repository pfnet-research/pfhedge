...
def main():
    ...
    experiments = [
        ...
        {'name': 'ES (p=0.1) - Extreme tail', 'loss': 'expected_shortfall', 'loss_param': 0.1},
        {'name': 'ES (p=0.3) - Upper tail', 'loss': 'expected_shortfall', 'loss_param': 0.3},
        {'name': 'ES (p=0.5) - Median (CURRENT BEST)', 'loss': 'expected_shortfall', 'loss_param': 0.5},
        {'name': 'ES (p=0.7) - Lower tail', 'loss': 'expected_shortfall', 'loss_param': 0.7},
        {'name': 'ES (p=0.9) - Near mean', 'loss': 'expected_shortfall', 'loss_param': 0.9},

        # EntropicRiskMeasure with different risk aversions
        {'name': 'Entropic (a=0.1) - Low aversion', 'loss': 'entropic', 'loss_param': 0.1},
        {'name': 'Entropic (a=0.5) - Medium-low', 'loss': 'entropic', 'loss_param': 0.5},
        {'name': 'Entropic (a=1.0) - Medium', 'loss': 'entropic', 'loss_param': 1.0},
        {'name': 'Entropic (a=2.0) - High aversion', 'loss': 'entropic', 'loss_param': 2.0},
        ...
        {'name': 'QuadCVaR (lam=1.5) - Mild', 'loss': 'quadratic_cvar', 'loss_param': 1.5},
        {'name': 'QuadCVaR (lam=2.0) - Medium', 'loss': 'quadratic_cvar', 'loss_param': 2.0},
        {'name': 'QuadCVaR (lam=5.0) - Strong', 'loss': 'quadratic_cvar', 'loss_param': 5.0},
        {'name': 'QuadCVaR (lam=10.0) - Very strong', 'loss': 'quadratic_cvar', 'loss_param': 10.0},
    ]
    ...
    if results:
        ...
        if wins:
            ...
            current_best = next((r for r in results if r['name'] == 'ES (p=0.5) - Median (CURRENT BEST)'), None)
