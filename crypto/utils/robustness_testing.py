...
def run_experiment(config, scenario):
    ...
    rel_improvement = (improvement / abs(results['Black-Scholes']['mean'])) * 100 if results['Black-Scholes']['mean'] != 0 else 0
...
def main():
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        print(f"  Strike: ${scenario['strike']}, Maturity: {scenario['maturity_days']}d, Vol: {scenario['volatility']:.0%}")
        ...
        try:
            ...
            print(f"  Result: Deep ${result['deep']['mean']:.0f} vs BS ${result['bs']['mean']:.0f}")
            print(f"  Improvement: ${result['improvement']:+.0f} ({result['rel_improvement']:+.1f}%) {status}")
    ...
    for category in ['Strike', 'Maturity', 'Volatility']:
        ...
        for r in cat_results:
            ...
            print(f"{name:<30} ${improvement:>9.0f} {rel_improvement:>7.1f}% {status:>8}")
    ...
    if wins:
        ...
        print(f"  Improvement: ${best['improvement']:.2f} ({best['rel_improvement']:.1f}%)")
