...
def test_loss_function(loss_config):
    ...
    print(f"Training with {train_config['n_paths']} paths for {loss_config.get('n_epochs', 40)} epochs...")
    ...
    try:
        ...
        print(f"Training: {initial_loss:.2f} → {final_loss:.2f} ({improvement:.1f}% improvement)")
    except Exception as e:
    ...
    with torch.no_grad():
        ...
        bs_hedge_pnl = calculate_bs_hedge_pnl(spots, bs_delta, payoffs, base_config['cost'])
    ...
    print(f"\nResults:")
    print(f"  Deep:  ${deep_mean:>8.2f} ± ${deep_std:>8.2f} (Sharpe: {deep_sharpe:>6.3f})")
...
def run_loss_function_experiments():
    ...
    if results:
        ...
        # Save results
        import json
        output_file = os.path.join(os.path.dirname(__file__), 'loss_function_results.json')
